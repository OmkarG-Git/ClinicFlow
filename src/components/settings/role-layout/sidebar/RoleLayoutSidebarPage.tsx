...
import { SIDEBAR_METADATA } from '@/constants/metadata/layout-metadata';
...
export function RoleLayoutSidebarPage({
    initialRole,
    initialLayout,
    ...
    useEffect(() => {
        if(layout && initialLayoutState) {
            const hasChanged = JSON.stringify(layout) !== JSON.stringify(initialLayoutState);
    ...
    const handleCancel = () => {
        setSidebarLayout(initialLayoutState);
        setHasChanges(false);
    }
    ...
    async function handleRoleChange(role: "DOCTOR" | "RECEPTIONIST") {
        ...
        startTransition(async () => {
            try {
                const response = await getRoleOptionLayoutAction(role, "sidebar");
                if (response.success) {
    ...
    try {
       const result = await updateRoleLayoutAction(
        ...
        if(result.success) {
            ...
            setSidebarLayout(result.data?.sidebar)
            ...
            <div className="relative">
                ...
                <div className={cn(
                    ...
                    <div className="mt-6">
                        <SectionContainer
                            title='Sidebar'
                            description='Control which sidebar items are visible.' 
                            layout={layout}
